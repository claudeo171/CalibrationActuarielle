...
namespace OnlineCalibrator.Batch
{
    ...
    /// 
    [Serializable]
    public class KSDistribution : UnivariateContinuousDistribution, IFormattable
    {
        ...
        /// 
        public KSDistribution([Positive] double samples)
        {
            if (samples <= 0)
            {
        ...
        {
            var toto= CumulativeFunction(NumberOfSamples, x);
            if(double.IsNaN(toto))
            {
        ...
        {
            var toto=ComplementaryDistributionFunction(NumberOfSamples, x);
            if(double.IsNaN(toto))
            {
        ...
        /// </summary>
        /// 
        public override void Fit(double[] observations, double[] weights, Accord.Statistics.Distributions.Fitting.IFittingOptions options)
        {
            throw new NotSupportedException();
        }
